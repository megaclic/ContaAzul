// ============================================================================
// ContaAzul Sync Produtos - Supabase Edge Function
// Version: 1.0.0
// Purpose: Sync produtos from ContaAzul API to local database
// ============================================================================

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.39.0';
import { createHash } from 'https://deno.land/std@0.168.0/hash/mod.ts';

// ============================================================================
// TYPES
// ============================================================================

interface SyncRequest {
  operation: 'full' | 'incremental'; // full = sync all, incremental = sync only changes
  force?: boolean; // force sync even if last sync was recent
}

interface SyncResult {
  success: boolean;
  job_id?: string;
  operation: string;
  total_fetched: number;
  total_upserted: number;
  total_errors: number;
  duration_ms: number;
  errors?: string[];
}

interface Produto {
  id: string;
  nome: string;
  valor: number;
  codigo?: string;
  tipo?: string;
  situacao?: string;
  formato?: string;
  categoria?: {
    id: string;
    descricao: string;
  };
  estoque?: {
    minimo?: number;
    maximo?: number;
    atual?: number;
  };
  data_cadastro?: string;
  data_alteracao?: string;
  [key: string]: any; // Allow other fields
}

// ============================================================================
// HELPER FUNCTIONS
// ============================================================================

/**
 * Decrypt token using AES-GCM
 */
async function decryptToken(encryptedToken: string): Promise<string> {
  const encoder = new TextEncoder();
  const decoder = new TextDecoder();
  
  const keyMaterial = encoder.encode(Deno.env.get('ENCRYPTION_KEY') || 'default-key-change-in-production');
  
  const key = await crypto.subtle.importKey(
    'raw',
    keyMaterial,
    { name: 'AES-GCM', length: 256 },
    false,
    ['decrypt']
  );
  
  const combined = Uint8Array.from(atob(encryptedToken), c => c.charCodeAt(0));
  const iv = combined.slice(0, 12);
  const encrypted = combined.slice(12);
  
  const decrypted = await crypto.subtle.decrypt(
    { name: 'AES-GCM', iv },
    key,
    encrypted
  );
  
  return decoder.decode(decrypted);
}

/**
 * Compute SHA-256 hash of object for change detection
 */
function computeHash(obj: any): string {
  const normalized = JSON.stringify(obj, Object.keys(obj).sort());
  const hash = createHash('sha256');
  hash.update(normalized);
  return hash.toString('hex');
}

/**
 * Get valid access token (refresh if needed)
 */
async function getValidAccessToken(supabase: any): Promise<string> {
  const { data: connection } = await supabase
    .from('contaazul_connections')
    .select('*')
    .eq('is_active', true)
    .single();

  if (!connection) {
    throw new Error('No active connection found');
  }

  // Check if token is expired or expiring soon (< 5 min)
  const expiresAt = new Date(connection.token_expires_at);
  const fiveMinutesFromNow = new Date(Date.now() + 5 * 60 * 1000);

  if (expiresAt < fiveMinutesFromNow) {
    // Trigger refresh
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    
    const refreshResponse = await fetch(
      `${supabaseUrl}/functions/v1/contaazul-token-refresh`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${supabaseKey}`,
        },
        body: JSON.stringify({ connection_id: connection.id }),
      }
    );

    if (!refreshResponse.ok) {
      throw new Error('Failed to refresh token');
    }

    // Re-fetch connection with new token
    const { data: refreshedConnection } = await supabase
      .from('contaazul_connections')
      .select('access_token_enc')
      .eq('id', connection.id)
      .single();

    return await decryptToken(refreshedConnection.access_token_enc);
  }

  return await decryptToken(connection.access_token_enc);
}

/**
 * Fetch produtos from ContaAzul API
 */
async function fetchProdutos(
  accessToken: string,
  operation: 'full' | 'incremental',
  lastSyncDate?: string
): Promise<Produto[]> {
  const baseUrl = 'https://api-v2.contaazul.com/v1/produtos';
  const allProdutos: Produto[] = [];
  
  let page = 1;
  let hasMore = true;

  while (hasMore) {
    const params = new URLSearchParams({
      page: page.toString(),
      size: '100', // Max page size
    });

    // For incremental sync, filter by data_alteracao
    if (operation === 'incremental' && lastSyncDate) {
      params.append('data_alteracao_inicial', lastSyncDate);
    }

    const url = `${baseUrl}?${params}toString()}`;

    const response = await fetch(url, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Accept': 'application/json',
      },
    });

    if (!response.ok) {
      const error = await response.text();
      throw new Error(`Failed to fetch produtos: ${error}`);
    }

    const data = await response.json();
    
    if (data && Array.isArray(data)) {
      allProdutos.push(...data);
      hasMore = data.length === 100; // If full page, there might be more
      page++;
    } else {
      hasMore = false;
    }
  }

  return allProdutos;
}

/**
 * Upsert produto to database
 */
async function upsertProduto(
  supabase: any,
  produto: Produto,
  hash: string
): Promise<void> {
  await supabase.from('contaazul_raw_produtos').upsert({
    id: produto.id,
    nome: produto.nome,
    valor: produto.valor,
    codigo: produto.codigo,
    tipo: produto.tipo,
    situacao: produto.situacao,
    formato: produto.formato,
    categoria_id: produto.categoria?.id,
    categoria_descricao: produto.categoria?.descricao,
    estoque_minimo: produto.estoque?.minimo,
    estoque_maximo: produto.estoque?.maximo,
    estoque_atual: produto.estoque?.atual,
    data_cadastro: produto.data_cadastro,
    data_alteracao: produto.data_alteracao,
    raw_payload: produto,
    payload_hash: hash,
    last_synced_at: new Date().toISOString(),
  }, {
    onConflict: 'id',
  });
}

// ============================================================================
// MAIN HANDLER
// ============================================================================

serve(async (req: Request) => {
  const corsHeaders = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type',
  };

  if (req.method === 'OPTIONS') {
    return new Response('ok', { headers: corsHeaders });
  }

  const startTime = Date.now();

  try {
    const supabaseUrl = Deno.env.get('SUPABASE_URL')!;
    const supabaseKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const supabase = createClient(supabaseUrl, supabaseKey);

    const body: SyncRequest = await req.json();
    const operation = body.operation || 'incremental';

    console.log(`Starting ${operation} sync for produtos...`);

    // ========================================================================
    // Create sync job record
    // ========================================================================

    const { data: job, error: jobError } = await supabase
      .from('contaazul_sync_jobs')
      .insert({
        entity_type: 'produtos',
        operation_type: operation,
        status: 'running',
        started_at: new Date().toISOString(),
      })
      .select()
      .single();

    if (jobError) {
      throw new Error(`Failed to create job: ${jobError.message}`);
    }

    const jobId = job.id;

    try {
      // ====================================================================
      // Get last sync date for incremental
      // ====================================================================

      let lastSyncDate: string | undefined;

      if (operation === 'incremental') {
        const { data: lastJob } = await supabase
          .from('contaazul_sync_jobs')
          .select('completed_at')
          .eq('entity_type', 'produtos')
          .eq('status', 'success')
          .order('completed_at', { ascending: false })
          .limit(1)
          .single();

        if (lastJob?.completed_at) {
          lastSyncDate = lastJob.completed_at;
          console.log(`Incremental sync from: ${lastSyncDate}`);
        } else {
          console.log('No previous sync found, falling back to full sync');
          operation = 'full';
        }
      }

      // ====================================================================
      // Get valid access token
      // ====================================================================

      const accessToken = await getValidAccessToken(supabase);

      // ====================================================================
      // Fetch produtos from API
      // ====================================================================

      console.log('Fetching produtos from ContaAzul API...');
      const produtos = await fetchProdutos(accessToken, operation, lastSyncDate);
      console.log(`Fetched ${produtos.length} produtos`);

      // ====================================================================
      // Upsert to database with hash-based change detection
      // ====================================================================

      console.log('Upserting produtos to database...');
      let upserted = 0;
      const errors: string[] = [];

      for (const produto of produtos) {
        try {
          const hash = computeHash(produto);
          await upsertProduto(supabase, produto, hash);
          upserted++;
        } catch (error) {
          console.error(`Error upserting produto ${produto.id}:`, error);
          errors.push(`${produto.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }

      // ====================================================================
      // Update job status
      // ====================================================================

      const duration = Date.now() - startTime;

      await supabase
        .from('contaazul_sync_jobs')
        .update({
          status: errors.length > 0 ? 'partial_success' : 'success',
          completed_at: new Date().toISOString(),
          records_processed: produtos.length,
          records_success: upserted,
          records_error: errors.length,
          error_details: errors.length > 0 ? errors : null,
          duration_ms: duration,
        })
        .eq('id', jobId);

      console.log(`Sync completed in ${duration}ms`);

      const result: SyncResult = {
        success: true,
        job_id: jobId,
        operation,
        total_fetched: produtos.length,
        total_upserted: upserted,
        total_errors: errors.length,
        duration_ms: duration,
        errors: errors.length > 0 ? errors : undefined,
      };

      return new Response(
        JSON.stringify(result),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        }
      );

    } catch (error) {
      // Update job status to error
      await supabase
        .from('contaazul_sync_jobs')
        .update({
          status: 'error',
          completed_at: new Date().toISOString(),
          error_details: [error instanceof Error ? error.message : 'Unknown error'],
          duration_ms: Date.now() - startTime,
        })
        .eq('id', jobId);

      throw error;
    }

  } catch (error) {
    console.error('Sync error:', error);

    return new Response(
      JSON.stringify({
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error',
      }),
      {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      }
    );
  }
});
